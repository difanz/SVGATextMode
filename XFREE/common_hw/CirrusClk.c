...
/* [kmg] !!! modified for CL-GD546x (RDRAM) chips */

int CirrusSetClock(freq, islaguna)
	int freq;
	Bool islaguna;
{
	int num, den, usemclk;
	...
	if (islaguna)  /* swap den and num for laguna chips */
	  CirrusFindClock(freq, MAX_VCO, &den, &num, &usemclk);
	else
